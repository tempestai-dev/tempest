...
**Database Branches** (shipping now)

Parallel agents doing real backend work need real databases. Tempest now ships per-project Docker-based database isolation: each project gets its own branched container to migrate, modify, or break without touching production or other sessions. Accessible from **Project Settings → Database** and visible in the right sidebar's DB Clones panel.
