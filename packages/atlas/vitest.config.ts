...
export default defineConfig({
  test: {
     ...
     * mcp-roots) spawn `dist/bin/atlas.js serve --mcp` with `process.execPath`
     * and rely on the child inheriting `process.env`. On a Node >= 25 dev machine
     * the CLI's hard-block (src/bin/atlas.ts) would otherwise exit the child
     ...
     * users (it's still enforced when `atlas` is invoked directly) while
     ...
     */
    env: {
      ATLAS_ALLOW_UNSAFE_NODE: '1',
      /**
       * The suite spawns real CLI/MCP processes; without this they would write
       * telemetry state into the contributor's real ~/.atlas and count test
