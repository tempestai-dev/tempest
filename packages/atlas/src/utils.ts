 ...
 * @example
 * ```typescript
 * import { Mutex, processInBatches, MemoryMonitor, validatePathWithinRoot } from 'atlas';
 *
 ...
 * API key, JDBC URL with embedded creds). Atlas must surface the KEY only
 ...
 * (atlas_node `includeCode`, atlas_explore source) go through here, so
  ...
  acquire(): void {
    ...
    if (fs.existsSync(this.lockPath)) {
      try {
        ...
        if (lockAge < FileLock.STALE_TIMEOUT_MS && !isNaN(pid) && this.isProcessAlive(pid)) {
          throw new Error(
            `Atlas database is locked by another process (PID ${pid}). ` +
            `If this is stale, run 'atlas unlock' or delete ${this.lockPath}`
          );
        }
    ...
    try {
      ...
      if (err.code === 'EEXIST') {
        ...
        throw new Error(
          'Atlas database is locked by another process. ' +
          `If this is stale, run 'atlas unlock' or delete ${this.lockPath}`
        );
      }
      throw err;
