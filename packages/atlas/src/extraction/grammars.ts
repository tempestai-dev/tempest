...
/**
 * Whether a file is one Atlas can parse, based purely on its extension.
 ...
 * `atlas.json`); when present its extensions count as indexable in addition
...
export async function loadGrammarsForLanguages(languages: Language[]): Promise<void> {
  ...
  for (const lang of toLoad) {
    ...
    try {
      ...
      console.warn(`[Atlas] Failed to load ${lang} grammar — parsing will be unavailable: ${message}`);
 ...
 *
 * `overrides` is the project's validated custom extension → language map (from
 * `atlas.json`); when present its mappings take precedence over the built-in
