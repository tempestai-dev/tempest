 ...
 * languages/frameworks Atlas supports — curated from the canonical
 ...
 * — IDE/state dirs like `.idea`/`.vs` are omitted because Atlas indexes only
 ...
 * slowing indexing, and skewing both the file count and `atlas_explore`
...
function readGitignorePatterns(giPath: string): string {
  ...
  try {
    ignore().add(content).ignores('.atlas-probe');
    return content;
  } catch {
  ...
  for (const line of content.split(/\r?\n/)) {
    try {
      ignore().add(line).ignores('.atlas-probe');
      kept.push(line);
    } catch {
      dropped++;
    }
...
/**
 * Matcher for the project's `atlas.json` `includeIgnored` patterns — the
...
/**
 * Matcher for the project's `atlas.json` `exclude` patterns — paths to keep
 ...
 *   of a repo Atlas already indexes — indexing it just duplicates the whole
 ...
 * from npm git-dependencies — that never makes it project code) and Atlas
...
function findNestedGitRepos(absDir: string, relPrefix: string): string[] {
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.name === '.git' || isAtlasDataDir(entry.name)) continue;
...
export class ScopeIgnore {
  ...
  constructor(
    ...
    /**
     * Project `atlas.json` `exclude` patterns (#999), matched against the
 ...
 *      NOT opt that path in via `atlas.json` `includeIgnored`. The gitignore
 ...
 * (#514) only for directories the project opted in via `atlas.json`
 ...
 * tool (and Atlas's own `git ls-files` foundation) does — `.gitignore`
 * excludes. So this returns `[]` unless the project opted the directory in via
 * `atlas.json` `includeIgnored`; without that, a gitignored dir — including
 ...
 * directories the project opted in through `atlas.json` `includeIgnored`
...
function collectGitFiles(repoDir: string, prefix: string, files: Set<string>, embeddedRoots?: Set<string>, includeIgnored: Ignore | null = null): void {
  ...
  // only when the project opted the directory in via `atlas.json`
 ...
 * directories opted in via `atlas.json` `includeIgnored` (#622, #699) —
 ...
 * with it); a full `atlas index` reconciles that.
 */
function getGitChangedFiles(rootDir: string): GitChanges | null {
  try {
    const changes: GitChanges = { modified: [], added: [], deleted: [] };
    // Custom extension → language overrides from the project's atlas.json,
...
function collectGitStatus(repoDir: string, prefix: string, out: GitChanges, overrides?: Record<string, Language>, includeIgnored: Ignore | null = null, exclude: Ignore | null = null): void {
  ...
  // change — so `atlas status` (which reads getChangedFiles) reports a
  ...
  for (const line of output.split('\n')) {
    ...
    // User `atlas.json` `exclude` (#999) is project-root-relative, so it's
...
function scanDirectoryWalk(
  ...
  function walk(dir: string, matchers: ScopedIgnore[]): void {
    ...
    for (const entry of entries) {
      // Never descend into git internals or any Atlas data directory
  ...
  // Project `atlas.json` `exclude` patterns (#999), rooted at the project so
...
export class ExtractionOrchestrator {
  ...
  async indexAll(
    ...
    if (useWorker) {
      // ATLAS_PARSE_WORKERS: explicit worker count; 1 = the old single-worker
      // behaviour (the conservative rollback). Unset → clamp(cores-1, 1, 8).
      const poolSize = resolveParsePoolSize(process.env.ATLAS_PARSE_WORKERS, os.cpus().length);
     ...
     * here on the main thread, where the atlas.json overrides are loaded.
     */
  ...
  async indexFileWithContent(
    ...
    // Detect language (honoring the project's atlas.json extension overrides)
