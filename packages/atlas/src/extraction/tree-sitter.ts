...
export class TreeSitterExtractor {
  ...
  // Value-reference edges (default ON; set ATLAS_VALUE_REFS=0 to disable; see flushValueRefs).
  ...
  private readonly valueRefsEnabled = process.env.ATLAS_VALUE_REFS !== '0';
   ...
   * function_ref), deduped per (reader, target). Default on (ATLAS_VALUE_REFS=0 disables) +
   ...
   * `atlas query query_apply_record` returned nothing (issue #634). We emit
