...
export class ContextBuilder {
  ...
  private buildLowConfidenceNote(entryPoints: Node[]): string {
    ...
    const dirLine = dirs.length
      ? `\n- \`atlas_files\` a likely area: ${dirs.map(d => `\`${d}\``).join(', ')}`
      : '';
    return `\n\n${LOW_CONFIDENCE_MARKER}\n\n`
      ...
      + '- `atlas_explore` with the **exact symbol names** you are after '
      + '(class / function / method names), or\n'
      + '- `atlas_search <name>` for one specific symbol'
      + dirLine
   ...
   * truncation is honest, and the agent can atlas_node the last hop to bridge.
   */
  private buildCallPathsSection(subgraph: Subgraph): string {
    ...
    const lines = [
      ...
      '',
      hasSynth
        ? '_Hops marked `[callback/event …]` are dynamic dispatch bridged by atlas (with the registration site); the rest are direct calls. atlas_node any symbol for its body._'
        : '_atlas_node any symbol above for its source + its own callers/callees._',
    ];
    return '\n' + lines.join('\n') + '\n';
  }
