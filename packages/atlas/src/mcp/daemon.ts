 ...
 *
 * One detached `atlas serve --mcp` daemon process per project root,
 ...
 *     `ATLAS_DAEMON_IDLE_TIMEOUT_MS` (default 300s) so back-to-back agent
 ...
 *   - The lockfile (`.atlas/daemon.pid`) competing daemons arbitrate
...
import { AtlasPackageVersion } from './version';
...
export interface DaemonHello {
  atlas: string; // package version (must match the proxy's own version)
 ...
 * lifecycle. The `atlas_client` marker is what tells it apart from the
...
export class Daemon {
  ...
  constructor(
    ...
    // (one stdio client) leaves the pool off; `ATLAS_QUERY_POOL_SIZE=0`
  ...
  async start(): Promise<DaemonStartResult> {
    ...
    const listen = (socketPath: string): Promise<net.Server> =>
      new Promise<net.Server>((resolve, reject) => {
        ...
        server.listen(socketPath, () => {
          ...
          // `.atlas/` (git-ignored, maybe a shared FS) or tmpdir.
          if (process.platform !== 'win32') {
      ...
      bound = await bindFirstUsableSocket(candidates, listen, {
        onRelocate: (from, to, code) =>
          process.stderr.write(
            `[Atlas daemon] Socket ${from} unusable (${code}); relocating to ${to}.\n`
          ),
      });
    } catch (err) {
    ...
    // Drop a discovery record so `atlas list` / `stop --all` can find us.
    // Best-effort; a missing record only means list's liveness prune covers it.
    registerDaemon({ root: this.projectRoot, ...lock });

    process.stderr.write(
      `[Atlas daemon] Listening on ${this.socketPath} (pid ${process.pid}, v${AtlasPackageVersion}). Idle timeout ${this.idleTimeoutMs}ms.\n`
    );
  ...
  async stop(reason: string = 'stop'): Promise<void> {
    ...
    process.stderr.write(`[Atlas daemon] Shutting down (${reason}; clients=${this.clients.size}).\n`);
  ...
  reapDeadClients(isAlive: (pid: number) => boolean): number {
    ...
    for (const session of [...this.clients]) {
      ...
      process.stderr.write(
        `[Atlas daemon] Reaping client with dead peer (pid ${peers.pid}); clients=${this.clients.size - 1}.\n`
      );
      try { session.stop(); } catch { /* best-effort */ }
...
export function tryAcquireDaemonLock(projectRoot: string): AcquireResult {
  ...
  // Make sure the .atlas/ directory exists — the daemon may be the first
...
function resolveIdleTimeoutMs(): number {
  const raw = process.env.ATLAS_DAEMON_IDLE_TIMEOUT_MS;
...
function resolveMaxIdleMs(): number {
  const raw = process.env.ATLAS_DAEMON_MAX_IDLE_MS;
...
function resolveClientSweepMs(): number {
  const raw = process.env.ATLAS_DAEMON_CLIENT_SWEEP_MS;
 ...
 * client-hello (carries the `atlas_client` marker), or null otherwise — in
...
export function parseClientHelloLine(
  ...
  if (o.atlas_client !== 1 || typeof o.pid !== 'number') return null;
