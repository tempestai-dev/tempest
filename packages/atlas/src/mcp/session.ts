 ...
 * resources (Atlas, watcher, ToolHandler) live in the shared
...
export class MCPSession {
  ...
  private async handleInitialize(request: JsonRpcRequest): Promise<void> {
    ...
    // `.atlas/`. Gating tool AVAILABILITY on whether `./` is indexed was the
    // #964 bug — it broke monorepos (only sub-projects indexed) and never
    // surfaced the tools after a mid-session `atlas init`. When no explicit
    // path is known yet (roots/list dance pending), cwd is the best predictor of
    // where the default project will resolve.
    const indexed = findNearestAtlasRoot(explicitPath ?? process.cwd()) !== null;
  ...
  private async handleToolsList(request: JsonRpcRequest): Promise<void> {
    ...
    // monorepo case where only sub-projects carry a `.atlas/` (the agent
    ...
    // the user ran `atlas init` (most hosts request the list once, so the
    ...
    // ("pass projectPath / run atlas init"), never `isError`, so it can't
    // teach the agent to abandon atlas. `getTools()` returns the default
   ...
   *      that were `atlas init`'d *after* the server started.
   */
  private async retryInitIfNeeded(): Promise<void> {
    ...
    if (this.engine.hasDefaultAtlas()) return;
  ...
  private async initFromRoots(): Promise<void> {
    ...
    try {
      ...
      if (rootPath) {
        ...
        process.stderr.write('[Atlas MCP] Client returned no workspace roots; falling back to process cwd.\n');
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[Atlas MCP] roots/list request failed (${msg}); falling back to process cwd.\n`);
    }
