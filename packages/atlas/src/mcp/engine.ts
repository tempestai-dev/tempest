 ...
 * the project's {@link Atlas} instance, file watcher, and the
...
// Lazy-load the heavy Atlas chain (sqlite + query/graph/context layers) OFF
...
export interface MCPEngineOptions {
   ...
   * single call never pays a worker round-trip. `ATLAS_QUERY_POOL_SIZE=0`
...
export class MCPEngine {
  private cg: Atlas | null = null;
  private toolHandler: ToolHandler;
  // Project root we resolved to. Null until `ensureInitialized` succeeds
  // (or null forever if no .atlas/ ever turned up — that's a valid
   ...
   * mode only; honors `ATLAS_QUERY_POOL_SIZE`). Idempotent and best-effort:
  ...
  private maybeStartPool(root: string): void {
    ...
    const size = resolvePoolSize(process.env.ATLAS_QUERY_POOL_SIZE, os.cpus().length);
    if (size <= 0) {
      process.stderr.write('[Atlas MCP] Query pool disabled (ATLAS_QUERY_POOL_SIZE=0); serving reads in-process.\n');
      return;
    }
    try {
      this.queryPool = new QueryPool({ root, size });
      this.toolHandler.setQueryPool(this.queryPool);
      process.stderr.write(`[Atlas MCP] Query pool: up to ${size} worker thread(s) for concurrent reads.\n`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[Atlas MCP] Query pool unavailable (${msg}); serving reads in-process.\n`);
      this.queryPool = null;
    }
  }
  ...
  /** Whether the default project's Atlas is open. */
  hasDefaultAtlas(): boolean {
    return this.toolHandler.hasDefaultAtlas();
  }

  /**
   * Walk up from `searchFrom` to find the nearest `.atlas/` and open it.
  ...
  async ensureInitialized(searchFrom: string): Promise<void> {
    ...
    if (this.toolHandler.hasDefaultAtlas()) return;
    if (this.initPromise) {
  ...
  retryInitializeSync(searchFrom: string): void {
    ...
    const resolvedRoot = findNearestAtlasRoot(searchFrom);
    ...
    try {
      ...
      this.cg = loadAtlas().openSync(resolvedRoot);
      this.projectPath = resolvedRoot;
      this.toolHandler.setDefaultAtlas(this.cg);
      this.startWatching();
      this.catchUpSync();
  ...
  private async doInitialize(searchFrom: string): Promise<void> {
    ...
    const resolvedRoot = findNearestAtlasRoot(searchFrom);
    if (!resolvedRoot) {
      // No .atlas/ above searchFrom. Sessions may still discover one later via roots/list
    ...
    this.projectPath = resolvedRoot;
    try {
      this.cg = await loadAtlas().open(resolvedRoot);
      this.toolHandler.setDefaultAtlas(this.cg);
      this.startWatching();
      this.catchUpSync();
      this.maybeStartPool(resolvedRoot);
      ...
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[Atlas MCP] Failed to open project at ${resolvedRoot}: ${msg}\n`);
    }
  }

  /**
   * Start file watching on the active Atlas instance. Idempotent — the
  ...
  private startWatching(): void {
    ...
    if (disabledReason) {
      process.stderr.write(
        `[Atlas MCP] File watcher disabled — ${disabledReason}. ` +
        `The graph will not auto-update; run \`atlas sync\` (or install the git sync hooks via \`atlas init\`) to refresh.\n`
      );
    ...
    const debounceMs = parseDebounceEnv(process.env.ATLAS_WATCH_DEBOUNCE_MS);
    if (debounceMs !== undefined) {
      process.stderr.write(`[Atlas MCP] File watcher debounce: ${debounceMs}ms (ATLAS_WATCH_DEBOUNCE_MS)\n`);
    }
    ...
    const started = this.cg.watch({
      ...
      onSyncComplete: (result) => {
        if (result.filesChanged > 0) {
          process.stderr.write(
            `[Atlas MCP] Auto-synced ${result.filesChanged} file(s) in ${result.durationMs}ms\n`
          );
        }
      },
      onSyncError: (err) => {
        process.stderr.write(`[Atlas MCP] Auto-sync error: ${err.message}\n`);
      },
      onDegraded: (reason) => {
        ...
        // (`atlas sync` / git sync hooks).
        process.stderr.write(`[Atlas MCP] File watcher degraded — ${reason}\n`);
      },
    });

    this.watcherStarted = true;
    if (started) {
      process.stderr.write('[Atlas MCP] File watcher active — graph will auto-sync on changes\n');
    } else {
      process.stderr.write(
        '[Atlas MCP] File watcher unavailable on this platform — run `atlas sync` to refresh the graph after changes.\n'
      );
    }
  }
  ...
  private catchUpSync(): void {
    ...
    const p = cg
      ...
      .then((result) => {
        ...
        if (changed > 0) {
          process.stderr.write(`[Atlas MCP] Caught up ${changed} file(s) changed since last run\n`);
        }
      })
      .catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[Atlas MCP] Catch-up sync failed: ${msg}\n`);
      });
...
/**
 * Parse and clamp the ATLAS_WATCH_DEBOUNCE_MS env override.
 *
