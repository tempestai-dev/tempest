 ...
 *
 * Defines the tools exposed by the Atlas MCP server.
 */

import type Atlas from '../index';
import type { QueryPool } from './query-pool';
import { findNearestAtlasRoot } from '../directory';
// Lazy-load the heavy Atlas chain off the MCP startup path — see the same
// helper in engine.ts. ToolHandler must load to answer tools/list (static
// schemas), but it must NOT drag in sqlite/query layers before the daemon binds;
// Atlas is pulled in only when a tool actually opens a project. require() is
...
/**
 * An expected, recoverable "atlas can't serve this" condition — most
 ...
 * calling atlas entirely (observed repeatedly), which is exactly wrong
...
/**
 * Node kinds that contain other symbols. For these, `atlas_node` with
...
/**
 * Calculate the recommended number of atlas_explore calls based on project size.
...
/**
 * Adaptive output budget for `atlas_explore`, scaled to project size.
 *
...
/**
 * Whether `atlas_explore` should prefix source lines with their line
 ...
 *
 * Defaults ON. Set `ATLAS_EXPLORE_LINENUMS=0` to disable (used by the
 * A/B harness to measure the payload-cost vs. read-savings tradeoff).
 */
function exploreLineNumbersEnabled(): boolean {
  return process.env.ATLAS_EXPLORE_LINENUMS !== '0';
}

/**
 * Adaptive explore sizing (default ON). `atlas_explore` skeletonizes OFF-SPINE
 ...
 * tokio / django / vscode / gin. Set `ATLAS_ADAPTIVE_EXPLORE=0` to disable.
 */
function adaptiveExploreEnabled(): boolean {
  return process.env.ATLAS_ADAPTIVE_EXPLORE !== '0' && process.env.ATLAS_ADAPTIVE_EXPLORE !== 'false';
}
 ...
 *
 * `ATLAS_CATCHUP_GATE_TIMEOUT_MS` overrides the default; `0` restores the
...
function resolveCatchUpGateTimeoutMs(): number {
  const raw = process.env.ATLAS_CATCHUP_GATE_TIMEOUT_MS;
...
/**
 * Unique line-prefix for a per-file source section in atlas_explore output.
...
// Placeholder for atlas_explore's "Found N symbols across M files." line.
...
const SUMMARY_SENTINEL = '[[atlas-explore-summary]]';
...
export function formatStaleBanner(stale: PendingFile[]): string {
  ...
  return (
    ...
    'their atlas entries may be stale:\n' +
    lines.join('\n') +
 ...
 * already names the operator remedy (`atlas sync` / git hooks).
 */
export function formatDegradedBanner(reason: string | null): string {
  return (
    '⚠️ Atlas auto-sync is DISABLED — live file watching stopped, so the index is ' +
 ...
 * — so atlas advertises these even though `initialize` still negotiates the
...
const projectPathProperty: PropertySchema = {
  type: 'string',
  description: 'Absolute path to the project to query (or any directory inside it) — atlas uses the nearest .atlas/ index at or above that path. Omit to use this session\'s default project. Pass it to query a second codebase, or when the server root has no index of its own (e.g. a monorepo where only sub-projects are indexed, so there is no default project).',
};

/**
 * EVERY atlas tool is query-only: it reads the pre-built index and never
 ...
 * All Atlas MCP tools
 *
 * Designed for minimal context usage - use atlas_explore as the primary tool
...
export const tools: ToolDefinition[] = [
  {
    name: 'atlas_search',
    description: 'Quick symbol search by name. Returns locations only (no code). Use atlas_explore instead to get the actual source / understand an area in one call.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  {
    name: 'atlas_callers',
    description: 'List functions that call <symbol>. For the full flow, use atlas_explore.',
    inputSchema: {
      type: 'object',
      properties: {
        symbol: {
  ...
  {
    name: 'atlas_callees',
    description: 'List functions that <symbol> calls. For the full flow, use atlas_explore.',
    inputSchema: {
      type: 'object',
      properties: {
        symbol: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Symbol names, file names, or short code terms to explore (e.g., "AuthService loginUser session-manager", "GraphTraverser BFS impact traversal.ts"). For a flow question, name the symbols spanning the flow (e.g. "mutateElement renderScene"). A natural-language question works too — no prior atlas_search needed.',
        },
        maxFiles: {
          type: 'number',
 ...
 * repo, or a monorepo root whose `.atlas/` indexes live only in sub-projects
 ...
 * `ToolHandler.getTools()` in the no-Atlas case (the dynamic per-repo budget
 * note in a description only adds once `cg` is loaded; the schemas are static).
 */
export function getStaticTools(): ToolDefinition[] {
  const raw = process.env.ATLAS_MCP_TOOLS;
  if (!raw || !raw.trim()) {
    return tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^atlas_/, '')));
  }
  const allow = new Set(raw.split(',').map(s => s.trim().replace(/^atlas_/, '')).filter(Boolean));
  return allow.size ? tools.filter(t => allow.has(t.name.replace(/^atlas_/, ''))) : tools;
}

/**
 * The MCP tools served by DEFAULT (short names). Pared to ONLY `atlas_explore`
...
export class ToolHandler {
  // Cache of opened Atlas instances for cross-project queries
  private projectCache: Map<string, Atlas> = new Map();
  ...
  // .atlas/ it resolves to), so the up-to-two `git rev-parse` spawns run
  ...
  constructor(private cg: Atlas | null) {}
  ...
  /**
   * Update the default Atlas instance (e.g. after lazy initialization)
   */
  setDefaultAtlas(cg: Atlas): void {
    this.cg = cg;
  }
    ...
    try {
      ...
      if (outcome === 'timeout') {
        process.stderr.write(
          `[Atlas MCP] Catch-up reconcile still running after ${timeoutMs}ms; serving this tool call now and finishing the reconcile in the background (#905). ` +
          `Set ATLAS_CATCHUP_GATE_TIMEOUT_MS=0 to always wait for it.\n`
        );
      }
    } finally {
  ...
  /**
   * Whether a default Atlas instance is available
   */
  hasDefaultAtlas(): boolean {
    return this.cg !== null;
  }

  /**
   * Optional allowlist of exposed tools, parsed from the ATLAS_MCP_TOOLS
   * env var (comma-separated short names, e.g. "trace,search,node,context").
   * Unset/empty → every tool is exposed. Lets an operator (or an A/B harness)
   * trim the tool surface without rebuilding the client config; the ablated
   * tool is then truly absent from ListTools rather than merely denied on call.
   * Matching is on the short form, so "node" and "atlas_node" both work.
   */
  private toolAllowlist(): Set<string> | null {
    const raw = process.env.ATLAS_MCP_TOOLS;
    if (!raw || !raw.trim()) return null;
    const short = (s: string) => s.trim().replace(/^atlas_/, '');
    const set = new Set(raw.split(',').map(short).filter(Boolean));
    return set.size ? set : null;
  }

  /** Whether a tool name passes the ATLAS_MCP_TOOLS allowlist (if any). */
  private isToolAllowed(name: string): boolean {
    const allow = this.toolAllowlist();
    return !allow || allow.has(name.replace(/^atlas_/, ''));
  }

  /**
   * Get tool definitions with dynamic descriptions based on project size.
   * The atlas_explore tool description includes a budget recommendation
   * scaled to the number of indexed files. Honors the ATLAS_MCP_TOOLS
  ...
  getTools(): ToolDefinition[] {
    ...
    let visible = allow
      ? tools.filter(t => allow.has(t.name.replace(/^atlas_/, '')))
      : tools.filter(t => DEFAULT_MCP_TOOLS.has(t.name.replace(/^atlas_/, '')));
    ...
    try {
      ...
      //   what atlas_node + atlas_explore would have answered.
  ...
  /**
   * Get Atlas instance for a project
   *
   * If projectPath is provided, opens that project's Atlas (cached).
   * Otherwise returns the default Atlas instance.
   *
   * Walks up parent directories to find the nearest .atlas/ folder,
   * similar to how git finds .git/ directories.
   */
  private getAtlas(projectPath?: string): Atlas {
    if (!projectPath) {
      if (!this.cg) {
        const searched = this.defaultProjectHint ?? process.cwd();
        throw new NotIndexedError(
          'No Atlas project is loaded for this session.\n' +
          `Searched for a .atlas/ directory starting from: ${searched}\n` +
          ...
          '(any project that has a .atlas/ — including a sub-project of a monorepo)\n' +
          '  • Or add --path to the server\'s MCP config args: ["serve", "--mcp", "--path", "/absolute/path/to/your/project"]\n' +
          'If a project simply has no index, use your built-in tools (Read/Grep/Glob) for THAT ' +
          "project (the user can run 'atlas init' there to enable it) — you can still query " +
    ...
    // real project must still be allowed to resolve UP to its .atlas/
    ...
    // Always RE-RESOLVE the nearest .atlas/ from the input path. The walk
    ...
    // importantly a git worktree that gained its own .atlas/ after the
    ...
    const resolvedRoot = findNearestAtlasRoot(projectPath);

    if (!resolvedRoot) {
      throw new NotIndexedError(
        `The project at ${projectPath} isn't indexed with atlas (no .atlas/ directory found ` +
        'walking up from it), so atlas cannot query it. Use your built-in tools (Read/Grep/Glob) ' +
        "for that codebase instead, and don't call atlas for it again this session. " +
        "Indexing is the user's decision — they can run 'atlas init' in that project to enable it."
      );
    }
    ...
    const cg = loadAtlas().openSync(resolvedRoot);
    this.projectCache.set(resolvedRoot, cg);
    return cg;
  }

  /**
   * Heal a long-lived connection whose `.atlas/` was removed and recreated
   * at the same path (a worktree recreated, or `rm -rf .atlas` + re-init)
   ...
   * tool call.
   */
  private freshen(cg: Atlas): Atlas {
    try {
      if (cg.reopenIfReplaced()) {
        process.stderr.write(
          '[Atlas MCP] The index was replaced on disk (e.g. a git worktree ' +
  ...
  private worktreeMismatchFor(projectPath?: string): WorktreeIndexMismatch | null {
    ...
    // (cheap — getAtlas re-walks to the nearest .atlas/, no git), then
    ...
    try {
      indexRoot = this.getAtlas(projectPath).getProjectRoot();
    } catch {
   ...
   * is no mismatch. `atlas_status` is excluded — it embeds its own verbose
  ...
  private withStalenessNotice(result: ToolResult, projectPath?: string): ToolResult {
    ...
    let cg: Atlas;
    try {
      cg = this.getAtlas(projectPath);
    } catch {
      return result; // no default project — leave as is
    }

    // Cross-project `projectPath` calls open a cached Atlas WITHOUT a
    ...
    // calls open a watcher-less Atlas, so this is false there — correct: we
    ...
    // Defensive: some test fakes inject a partial Atlas stub without the
    ...
    for (const p of pending) {
      ...
      // exactly the format both the watcher and every atlas response
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      // Honor the optional tool allowlist (ATLAS_MCP_TOOLS): a trimmed
      // surface rejects ablated tools defensively even if a client cached them.
      if (!this.isToolAllowed(toolName)) {
        return this.errorResult(`Tool ${toolName} is disabled via ATLAS_MCP_TOOLS`);
      }
      ...
      // The `path` and `pattern` properties used by atlas_files are
      ...
      // atlas_status reports watcher state (pending files, degraded mode,
      ...
      return this.errorResult(
        ...
        'This is an internal atlas error — retry the call once; if it persists, ' +
        'continue without atlas for this task.'
      );
    }
  }
   ...
   * allowlist/validation (the caller owns those). `atlas_status` is handled
  ...
  private async dispatchTool(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    switch (toolName) {
      case 'atlas_search': return await this.handleSearch(args);
      case 'atlas_callers': return await this.handleCallers(args);
      case 'atlas_callees': return await this.handleCallees(args);
      case 'atlas_impact': return await this.handleImpact(args);
  ...
  /**
   * Flow-from-named-symbols: an agent's atlas_explore query is a bag of
   ...
   */
  private buildFlowFromNamedSymbols(cg: Atlas, query: string): { text: string; pathNodeIds: Set<string>; namedNodeIds: Set<string>; uniqueNamedNodeIds: Set<string>; spineCallSites: Map<string, number> } {
   ...
   */
  private buildDynamicBoundaries(cg: Atlas, scanList: Node[], named: Map<string, Node>): string {
    ...
    return [
      ...
      ...notes,
      '',
      '> These sites choose their call target at runtime (registry / bus / reflection) — the site shown IS where the flow continues. To follow it, run atlas_explore or atlas_node on a candidate; source for the sites above is included below.',
      '',
    ].join('\n');
  }
   ...
   * then steer to atlas_explore. Graph-only, query-time, zero mutation; the
   ...
   */
  private buildPolymorphicBoundaries(cg: Atlas, candidates: Array<{ token: string; family: Node[] }>, named: Map<string, Node>): string {
    ...
    return [
      ...
      ...notes,
      '',
      '> The method above is dispatched at runtime to one of the listed implementations (a registry / plugin / strategy interface) — there is no single static caller→callee edge; the implementations ARE the continuations. To follow one, run atlas_explore on a listed target.',
      '',
    ].join('\n');
  }
   ...
   */
  private boundaryCandidates(cg: Atlas, key: string, keyIsType: boolean, named: Map<string, Node>, selfId: string): string {
   ...
   */
  private buildBlastRadiusSection(cg: Atlas, subgraph: Subgraph): string {
   ...
   * atlas's home turf: relevance by STRUCTURE, not words. A file whose
  ...
  /**
   * Handle atlas_explore — deep exploration in a single call
   *
   * Strategy: find relevant symbols via graph traversal, group by file,
   * then read contiguous file sections covering all symbols per file.
   * This replaces multiple atlas_node + Read calls.
   *
  ...
  private async handleExplore(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    const tierSeedIds = new Set<string>();
    {
      ...
      for (const t of tokens) {
        ...
        // atlas_node's findSymbolMatches.) Qualified tokens keep findAllSymbols.
        const isQual = /[.\/]|::/.test(t);
        ...
        // explore-side mirror of atlas_node's overload disambiguation.
        let picks: Node[];
    ...
    // cluster are unaffected (no competing mass). Set ATLAS_RANK_NO_MULTITERM=1
    // to disable.
    const MULTITERM_OFF = process.env.ATLAS_RANK_NO_MULTITERM === '1';
    ...
    for (const [filePath, group] of sortedFiles) {
      ...
      // Adaptive sizing (ATLAS_ADAPTIVE_EXPLORE, default on): collapse a file
      ...
      if (adaptiveExploreEnabled() && flow.pathNodeIds.size > 0
        ...
        if (skel.length > 0) {
          ...
          // Steer the agent to atlas_explore for an elided body — NEVER to
          ...
          const tag = bodyIds.size > 0
            ? 'focused (the methods you named in full, the rest as signatures — atlas_explore a signature by name for its body; do NOT Read)'
            : 'skeleton (signatures only — atlas_explore a name for its full body; do NOT Read)';
    ...
    if (budget.includeCompletenessSignal) {
      lines.push('');
      lines.push('---');
      lines.push(`> **Complete source for ${filesIncluded} files is included above — do NOT re-read them.** If your question also needs files/symbols listed under "Not shown above" (or any area this call didn't cover), make ANOTHER atlas_explore targeting those names — it returns the same source with line numbers and is cheaper and more complete than reading. Reserve Read for a single specific line range explore can't surface.`);
    } else if (anyFileTrimmed) {
      lines.push('');
      lines.push(`> Some file sections were trimmed for size. For a specific symbol you still need, run another \`atlas_explore\` (or \`atlas_node\`) with its exact name — line-numbered source, cheaper and more complete than Read.`);
    }
    ...
    if (output.length > hardCeiling) {
      ...
      finalText = safe + '\n\n... (output truncated to budget; the source above is complete and verbatim — treat it as already Read. For any area not covered, run another atlas_explore with the specific names — do NOT Read these files.)';
    } else {
      finalText = output;
    }
  ...
  private async handleNode(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    // atlas_node read cause on Swift/Go. So return them ALL: pack as many
    ...
    if (listed.length) {
      ...
      out.push(
        '',
        `> Need one of these in full? Call atlas_node again with \`file\` (e.g. \`"${listed[0]!.filePath.split('/').pop()}"\`) or \`line\` — do NOT Read it.`,
      );
    }
  ...
  private async handleFileView(
    ...
    if (allFiles.length === 0) return this.textResult('No files indexed. Run `atlas index` first.');
    ...
    if (!resolved) {
      return this.textResult(
        `No indexed file matches "${fileArg}". Atlas indexes source files; configs/docs it doesn't parse won't appear — Read those directly.`,
      );
    }
    ...
    // Compact, one-line blast radius (atlas's value-add over a plain Read).
    ...
    if (CONFIG_LEAF_LANGUAGES.has(resolved.language)) {
      ...
      out.push('', '> Values may be secrets, so atlas indexes keys only. Read the file directly if you need a value.');
      return this.textResult(this.truncateOutput(out.join('\n')));
    }
    ...
    if (!complete) {
      out.push(
        '',
        `(lines ${offset}–${shownEnd} of ${total} — pass \`offset\`/\`limit\` for another range, or \`atlas_node <symbol>\` for one symbol in full)`,
      );
    }
  ...
  private async renderNodeSection(cg: Atlas, node: Node, includeCode: boolean): Promise<string> {
   ...
   * callers (what calls it), each with file:line — so atlas_node doubles as
   * the structural Grep→Read→expand primitive: a spot PLUS where to go next.
   * Capped to stay cheap. Walk the graph by calling atlas_node on a trail
   ...
   */
  private formatTrail(cg: Atlas, node: Node): string {
    const TRAIL_CAP = 12;
    ...
    const lines: string[] = ['', '**Trail — atlas_node any of these to follow it (no Read needed)**'];
    if (callees.length > 0) {
  ...
  private async handleStatus(args: Record<string, unknown>): Promise<ToolResult> {
    let cg = this.getAtlas(args.projectPath as string | undefined);
  ...
  /**
   * Handle atlas_files - get project file structure from the index
   */
  private async handleFiles(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (allFiles.length === 0) {
      return this.textResult('No files indexed. Run `atlas index` first.');
    }
  ...
  /**
   * Find ALL definitions matching a name, ranked, so atlas_node can return
   ...
   */
  private findSymbolMatches(cg: Atlas, symbol: string): Node[] {
   ...
   */
  private findAllSymbols(cg: Atlas, symbol: string): { nodes: Node[]; note: string } {
   ...
   */
  private buildContainerOutline(cg: Atlas, node: Node): string {
  ...
  private formatNodeDetails(node: Node, code: string | null, outline?: string | null): string {
    ...
    if (outline) {
      lines.push('', outline, '',
        `> Structural outline only. Read \`${node.filePath}\` or call atlas_node on a specific member for its body.`);
    } else if (code) {
      // Line-numbered (cat -n style, like atlas_explore and Read) so the
