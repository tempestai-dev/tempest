 ...
 * Atlas MCP Server
 *
 * Model Context Protocol server that exposes Atlas functionality
 ...
 *   behavior; used when the user opts out (`ATLAS_NO_DAEMON=1`), no
 *   `.atlas/` is reachable, or the daemon machinery fails for any reason.
 ...
 *   sharing one Atlas + watcher + SQLite handle. Spawned on demand; never a
...
import { findNearestAtlasRoot, getAtlasDir } from '../directory';
 ...
 */
const DAEMON_INTERNAL_ENV = 'ATLAS_DAEMON_INTERNAL';
...
// are cheap. Paired with deferring the Atlas load (engine.ts) off the bind
...
/** Whether `ATLAS_NO_DAEMON` was set to a truthy value. */
function daemonOptOutSet(): boolean {
  const raw = process.env.ATLAS_NO_DAEMON;
  if (!raw) return false;
 ...
 * `null` when no `.atlas/` is reachable from the candidate path — in
 * that case the caller must run in direct mode, since the daemon lockfile
 * and socket both live under `.atlas/`.
 *
...
function resolveDaemonRoot(explicitPath: string | null): string | null {
  ...
  const root = findNearestAtlasRoot(candidate);
  if (!root) return null;
 ...
 * `.atlas/daemon.log`). Re-invokes the *same* CLI faithfully across dev and
...
function spawnDetachedDaemon(root: string): void {
  ...
  try {
    logFd = fs.openSync(path.join(getAtlasDir(root), 'daemon.log'), 'a');
 ...
 *
 * Implements the Model Context Protocol to expose Atlas
...
export class MCPServer {
   ...
   *
   * Decision order:
   *   1. `ATLAS_NO_DAEMON=1` → direct mode (unchanged pre-#411 behavior).
   *   2. `ATLAS_DAEMON_INTERNAL=1` → we ARE the detached daemon; listen.
   *   3. No `.atlas/` reachable → direct mode (the daemon's lockfile and
   *      socket both live under `.atlas/`).
  ...
  async start(): Promise<void> {
    ...
    if (daemonOptOutSet()) {
      return this.startDirect('ATLAS_NO_DAEMON set');
    }
    ...
    if (!root) {
      ...
      return this.startDirect('no .atlas/ root found');
    }
    ...
    try {
      ...
      process.stderr.write(`[Atlas MCP] Proxy path failed (${msg}); falling back to direct mode.\n`);
  ...
  private async startDirect(reason: string): Promise<void> {
    if (reason && process.env.ATLAS_MCP_DEBUG) {
      process.stderr.write(`[Atlas MCP] Direct mode: ${reason}.\n`);
    }
   ...
   * `ATLAS_DAEMON_INTERNAL=1`). Arbitrate the O_EXCL lock, then either
  ...
  private async startDaemonProcess(): Promise<void> {
    ...
    for (let attempt = 0; attempt < TAKEOVER_MAX_RETRIES; attempt++) {
      ...
      if (existing && existing.pid > 0 && isProcessAlive(existing.pid)) {
        process.stderr.write(
          `[Atlas daemon] Another daemon (pid ${existing.pid}) already holds the lock; exiting.\n`
        );
        process.exit(0);
      }
    ...
    process.stderr.write('[Atlas daemon] Could not acquire the daemon lock; exiting.\n');
    process.exit(0);
  }
  ...
  private installPpidWatchdog(): void {
    ...
    const pollMs = parsePpidPollMs(process.env.ATLAS_PPID_POLL_MS);
    if (pollMs <= 0) return;
    this.ppidWatchdog = setInterval(() => {
      ...
      if (reason) {
        process.stderr.write(
          `[Atlas MCP] Parent process exited (${reason}); shutting down.\n`
        );
        this.stop();
      }
    }, pollMs);
...
export { AtlasPackageVersion } from './version';
