 ...
 * Opt out with `ATLAS_NO_WATCHDOG=1`; tune with `ATLAS_WATCHDOG_TIMEOUT_MS`.
 */
...
function debug(msg: string): void {
  if (process.env.ATLAS_MCP_DEBUG) {
    try { fs.writeSync(2, `[Atlas watchdog] ${msg}\n`); } catch { /* ignore */ }
  }
}
...
const MSG = Buffer.from('[Atlas] Main thread unresponsive for ~' + secs + 's — killing the wedged process so a fresh one can start (#850). Disable with ATLAS_NO_WATCHDOG=1.\\n');
function kill() {
...
export function installMainThreadWatchdog(): WatchdogHandle | null {
  if (isEnvTruthy(process.env.ATLAS_NO_WATCHDOG)) return null;

  const timeoutMs = parseWatchdogTimeoutMs(process.env.ATLAS_WATCHDOG_TIMEOUT_MS);
