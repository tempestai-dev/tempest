 ...
 */
const LOG_ATTACH_ENV = 'ATLAS_MCP_LOG_ATTACH';
...
export function logAttachedDaemon(socketPath: string, hello: DaemonHello): void {
  ...
  process.stderr.write(
    `[Atlas MCP] Attached to shared daemon on ${socketPath} (pid ${hello.pid}, v${hello.atlas}).\n`
  );
}
...
export async function runProxy(
  socketPath: string,
  expectedVersion: string = AtlasPackageVersion,
): Promise<ProxyResult> {
  ...
  if (hello.atlas !== expectedVersion) {
    process.stderr.write(
      `[Atlas MCP] Found a daemon on ${socketPath} but version (${hello.atlas}) ` +
...
export async function runLocalHandshakeProxy(deps: LocalHandshakeDeps): Promise<void> {
  ...
  const handleLocally = async (line: string): Promise<void> => {
    ...
    if (msg.method === 'tools/call' && id !== undefined) {
      ...
      writeClient({ jsonrpc: '2.0', id, error: { code: -32603, message: 'Atlas daemon unavailable' } });
    }
  ...
  if (socket && !socket.destroyed && !shuttingDown) {
    ...
    // this host would silently lose Atlas and any in-flight request would
    ...
    const onDaemonLost = (): void => {
      ...
      daemonSocket = null;
      process.stderr.write(
        `[Atlas MCP] Shared daemon connection lost; serving this session in-process (degraded), re-serving ${inflight.size} in-flight request(s).\n`
      );
    ...
    daemonStatus = 'failed';
    process.stderr.write('[Atlas MCP] Shared daemon unavailable; serving this session in-process (degraded).\n');
...
function startPpidWatchdogNoSocket(onDeath: () => void): void {
  const pollMs = parsePollMs(process.env.ATLAS_PPID_POLL_MS);
  if (pollMs <= 0) return;
  ...
  const timer = setInterval(() => {
    ...
    if (reason) {
      process.stderr.write(`[Atlas MCP] Parent process exited (${reason}); shutting down.\n`);
      onDeath();
    }
  }, pollMs);
  timer.unref?.();
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: string | Buffer) => {
      ...
      try {
        ...
        if (typeof parsed.atlas !== 'string' || typeof parsed.pid !== 'number') {
...
function pipeUntilClose(socket: net.Socket): Promise<void> {
  return new Promise((resolve) => {
    ...
    socket.on('error', (err) => {
      process.stderr.write(`[Atlas MCP] daemon socket error: ${err.message}\n`);
      done();
    });
  });
}
