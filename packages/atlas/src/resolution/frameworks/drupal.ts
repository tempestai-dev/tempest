 ...
 *    canonical `hook_X` name, linking implementations to the hook when `atlas_callers`
 *    is invoked.
 *
 ...
 *   `atlas_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `atlas_callers`
